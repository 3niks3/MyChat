        ...
        <div class="col">
            ...
            <div class="card col-lg-6">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col">
                            <form id="chat_group_edit_form" action="{{route('chat_group_edit_receiver', $chat_group->id)}}" method="post" enctype="multipart/form-data">

                                <div class="row">
                                    <div class="col">
                                        <div class="form-floating mb-3">
                                            <input type="text" name="name" value="{{$chat_group->name}}" class="form-control" id="name" placeholder="Chat group name">
                                        ...
                                        <div class="form-floating mb-3">
                                            <select class="form-select" id="chat_group_category" name="chat_group_category" aria-label="Chat group category">
                                                ...
                                                @foreach($categories as $category)
                                                    <option value="{{$category->id}}" {{($chat_group->category == $category->id)?'selected':''}}>{{$category->name}}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-floating mb-3">
                                            <select class="form-select" id="chat_group_type" name="chat_group_type" aria-label="Chat group type">
                                                ...
                                                @foreach(config('chat-groups.chat-groups.types') as $type => $title)
                                                    <option value="{{$type}}" {{($chat_group->type == $type)?'selected':''}}>{{$title}}</option>
                                                @endforeach
                                            </select>
...
@push('scripts')
    <script>
        $('form#chat_group_edit_form').submit(function(e){

            e.preventDefault();
            let form =  $('form#chat_group_edit_form');
